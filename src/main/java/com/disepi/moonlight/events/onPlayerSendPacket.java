...
public class onPlayerSendPacket implements Listener {
    ...
    public void onPlayerPacketSend(DataPacketReceiveEvent event) {
        ...
        if ((event.getPacket() instanceof AnimatePacket)) {
            ...
            if (packet.eid != player.getId()) return;
        ...
        if ((event.getPacket() instanceof PlayerAuthInputPacket)) // If the received packet isn't PlayerAuthInputPacket then we don't want it right now
        {
            ...
            if (!Util.isRoughlyEqual(packet.getPosition().y % 0.015625f, 0.010627747f, 0.00001f))
                data.onGround = false;
